...
- **$\int (...) dP(L)$**: the integral over $dP(L)$ indicates the treatment effect is computed over all the probability distribution of $L$ in the population.
...
We here these words used freely in psychology.  What do they mean? 


This term *moderation* invites confusion. Let's set this to the side, and focus on two concepts: 

1. Interaction
2. Effect-modification

and let's define these concepts


### An interaction is a joint intervention

An "interaction" occurs is the combined effect of two interventions (or equivalently two treatments).  

Call these treatments, $A$ and $B$.  Call the outcome $Y$.

In potential outcomes terminology, we defined the potential outcome under treatment $A = \tilde{a}$:

$$
Y(\tilde{a})
$$

And we identify $Y(\tilde{a})$ from data using the consistency assumption: $Y_i(\tilde{a}) = (Y_i |A_i = \tilde{a})$, assuming that $Y_i(\tilde{a})\coprod A|L$, where $L$ donotes a set of measured covariates sufficient to insure no confounding (no common causes), or equivalently, no open backdoor path between $A$ and $Y$.


Interaction denotes a joint intervention of two treatments, call them $A$ and $B$, which may act on $Y$. The concept of a joint intervention requires that we extend our notation to:

$$
Y(\tilde{a}, \tilde{b})
$$

In addition to requiring $Y_i(\tilde{a}) = (Y_i \mid A_i = \tilde{a})$, assuming that $Y_i(\tilde{a}) \coprod A \mid L$, we also assume both that $Y_i(\tilde{b}) = (Y_i \mid B_i = \tilde{b})$, and that $Y_i(\tilde{b}) \coprod B \mid Q$, where $Q$ denotes a set of confounders sufficient to ensure no confounding for the effect of $B$ on $Y$. 
...
That is the union of $L$ and $Q$ does not imply the empty set. 
...
Consider the effect of beliefs in Big Gods (treatment, denoted  $A$) on social complexity (outcome, denoted$Y$), potentially influenced by a culture's monumental architecture (treatment, denoted $B$). Let's say we wish to assess the individual and combined effects of $A$ and $B$ on the difference scale.  Let assume the treatments are binary variables.
...
By adjusting for $Q_{0}$ we obtain an unbiased estimate of the $B\to Y$ path. As indicated in @fig-dag-interaction-solved, we must condition on both $L_{0}$ and $Q_{0}$ to identify causal interaction conceived as a joint interaction. [^noteinteraction]
...
Heterogeneous Treatment Effects (HTE) refer to settings where treatment effects vary across individuals or subgroups within a study population. This variability can arise from differences in baseline characteristics, environmental conditions, and potentially unmeasured covariates. 

**Appendix A** discusses the challenges in identifying HTE.  


For now, let us focus on the concept of heterogenous treatment effects as they apply to groups. 


### Conditional Average Treatment Effect (CATE)

If we are interested in estimating the causal effects in groups, we 

- **Focus**: Estimation of the average treatment effect conditional on a specific level of a covariate or set of covariates.
- **Estimand**: $\text{CATE}(x) = E[Y(1) - Y(0) | X = x]$, where $X = x$, where $X$ denotes the subgroup characteristics.

Question, how does the causal effect vary within levels of stratum $X=x$ 


### Effect-Modification


Effect Modification occurs when the effect of a treatment or intervention on an outcome varies across levels of a third variable, known as the effect-modifier. This third variable modifies the effect of the treatment, which can be observed by different effects in different subgroups defined by this modifier.  Again, it is scientifically interesting to consider whether treatment effects vary across levels of other variables without assuming a double intervention. 


We might be interested in the following causal quantity (estimand), which compares two conditional average treatment effects

$$\hat{\gamma} = \overbrace{\big( \hat{E}[Y(a^*)|G=g] - \hat{E}[Y(a)|G=g] \big)}^{\hat{\delta_g}} - \overbrace{\big(\hat{E}[Y(a^*)|G=g^{\prime}]- \hat{E}[Y(a)|G=g']\big)}^{\hat{\delta_{g^{\prime}}}}$$


Suppose $A$ is the treatment, $G$ is the effect-modifier, and $Y$ is the outcome. Effect-modification assesses whether the effect of $A$ on $Y$ is different across levels of $G$ (i.e., whether the effect of $A$ on $Y$ is different when $G = g_1$ compared to when $G = g_2$).

@fig-dag-effect-modification consider whether effect-modification of $A$ on $Y$ across levels of $G$. Because we are not interested in the causal effect of $G$ as such, but rather, how the effect of $A$ varies across $G$, we would not need to adjust $G$ by $Q$.  

However if we were to include either $Q$ or $Z$ in the model, we would attenuate the effect modification of $A$ by $G$.  Can you tell why? 


```{tikz}
#| label: fig-dag-effect-modification
#| fig-cap: "Imagine A is an experiment. There is no confounding, and there is an effect of A on Y in at least one stratum of the population.  Here, the marginal association between A and Y provides an unbiased estimate for the causal effect of A on Y. Does the conditional association of A on Y vary within levels of G? Yes, however G does not cause Y. Let us assume Q causes Y. What would be the effect of conditioning on Q if we are interested in assessing how A varies within levels of G?  What would happen if we were to condition on Z?"
#| out-width: 80%
#| echo: false
...
%\node [rectangle, draw=red, thick] (L) at (-4,0) {L};
\node [rectangle, draw=white, thick] (Q) at (-3,-1) {Q};
\node [rectangle, draw=white, thick] (A) at (-1,0) {A$_{1}$};

\node [emoddot] (Z) at (1, -1) {Z};
\node [black] (G) at (-1,-1) {G};

\node [rectangle, draw=none, thick] (Y) at (3,0) {Y$_{2}$};


%\draw[Arrow, draw=red] (L) to (A);
%\draw[Arrow, draw=red, bend left = 30] (L) to (Y);
\draw[Arrow, draw=black]   (G) to (Z);
\draw[Arrow, draw=black]   (Q) to (G);
\draw[Arrow, draw=black, bend right = 40] (Q) to (Y);
\draw [-latex, draw=black] (A) to (Y);
\draw[-{Circle[open, fill=none]}, line width=0.25pt, draw=blue] (Z) -- (Y); % Circle-ended arrow
\end{tikzpicture}
```



We may obtain causal effects by restricting to one level of a population. Where $G$ denotes a society, consider:




1.  **Causal effect within North American societies (**$G=1$):
    $$\delta_{g_1} = \mathbb{E}[Y(1)|g=1] - \mathbb{E}[Y(0)|g=1]$$ 
...
2.  **Causal effect within Continental societies (**$G=2$):
    ...
    $$\delta_{g2} = \mathbb{E}[Y(1)|g=2] - \mathbb{E}[Y(0)|g=2]$$
...
baseline_vars = c("age", "male", "edu", "eth_cat", "partner", "employed", "born_nz", "neighbourhood_community", "household_inc_log",
    "partner","parent", "religion_religious", "rural_gch_2018_l","sample_weights", "employed", "alert_level_combined_lead")
