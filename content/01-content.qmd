...
# note we use "as.integer()" not "integer()" because 
...
- **Simplicity:** Base R functions like `plot`, `hist`, `boxplot`, and others are straightforward to use for basic plotting tasks.
- **Speed:** For small to medium-sized datasets, Base R plots can be generated quickly, making them ideal for initial data exploration.
...
- `boxplot()`: Useful for comparing distributions across groups, highlighting medians, quartiles, and outliers.
