...
1. $Y(0, 0)$: The potential outcome when neither marriage nor remarriage occurs.
2. $Y(1, 0)$: The potential outcome when there is marriage but no remarriage.
3. $Y(0, 1)$: The potential outcome when there is no initial marriage, but there is remarriage. This condition may require additional clarification, as remarriage implies an initial marriage, suggesting a possible redefinition or clarification of treatment states for logical consistency.
4. $Y(1, 1)$: The potential outcome when both marriage and remarriage occur.
...
> We are like sailors who on the open sea must reconstruct their ship but are never able to start afresh from the bottom. Where a beam is taken away a new one must at once be put there, and for this the rest of the ship is used as support. In this way, by using the old beams and driftwood the ship can be shaped entirely anew, but only by gradual reconstruction. [@neurath1973]

This quotation emphasises the continuous and iterative process of knowledge development;  new insights are cast from the foundation of existing knowledge. Causal diagrams are at home in Neurath's boat. The tradition of science that believes that knowledge accumulates the results of statistical tests applied to data has done considerable harm and must be resisted. The data alone do not contain the answers we seek.
...
ANOVA partitions variance into between-group and within-group components, while regression models the mean of the dependent variable as a linear function of the independent (including categorical) variables. For many questions, ANOVA is appropriate, however, when comparing groups, we often want a finer-grained interpretation. Regression is built for obtaining this finer grain understanding. We will return to regression over the next few weeks and use regression to hone your skills in R. Later, Along the way, you'll learn more about data visualisation, modelling, and reporting.  
