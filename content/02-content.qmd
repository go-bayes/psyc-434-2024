...
5. Save the new R script file in your project directory for easy access during the lab -- noting that the script is for the second lab.
...
1. Psychological research typically begins with a question. What do we want to know? For whom does this knowledge apply?  Before attempting to answer a question, we must first ask it [@hernan2024WHATIF]
...
3.  The following concepts help us to describe two distinct failure modes in psychological research:
  ...
  - **Internal Validity**: the extent to which the associations we describe between the treatment and outcome reflect causality. In this course, we will use the terms "outcome" and "treatment" in place of "independent" and "dependent".[^note]

  [^note]: We use the terms "treatment" or "exposure" in place of "dependant variable" and we use "outcome" in place of "independent variable." We do so because "What if?" questions implicitly invoke the idea of intervening on the world. "If we did this, *then* what would happen to that...?" Our preferred terminology reflects our interet in the effects of interventions.

5.  Our focus in the the next two weeks:  challenges to internal validity from **confounding bias.**  


::: {#def-bias}
We say there is bias if the association between the treatment and the outcome does not reflect a causal association. 
:::

::: {#def-confounding}
We say there is confounding-bias if the there is an open back-door path between the treatment and outcome, or if that path between the treatment and outcome is blocked.
:::


Causal Diagrams, also called Causal Graphs, Directed Acyclic Graphs, and Causal Directed Acyclic Graphs, help us to detect confounding, and to develop strategies for addressing it to meet the demands of internal validity: the associations we obtain in our analysis consistently estimate causal associations for the target population.

## Introduction to Causal Diagrams -- or "Causal Directed Acyclic Graphs"
