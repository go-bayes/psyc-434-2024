...
Then check that you have the simulated data:
...
The variables in the simulated data correspond to a subset of the variables in this directory
...
dat_long_wide <- margot::margot_wide(dat_long, baseline_vars = baseline_vars, exposure_var = exposure_var, outcome_vars =outcome_vars)
...
dt<- remove_numeric_attributes(dt)
  
```

#### Ordinary regression to obtain conditional average treatment effect

```{r}
fit_1 <- lm(t2_kessler6_sum_z  ~ t1_perfectionism_z + t0_age_z +  t0_edu_z + t0_kessler6_sum_z + t0_perfectionism_z, data = dt)
...
#| echo: true
#| eval: false
fit_ate <- stdReg::stdGlm(fit_1, data = dt, X = "t1_perfectionism_z")
